...
function formatCurrency(
  ...
  if (currency === "UZS") {
    return `${amount.toLocaleString()} ${sumLabel}`;
  }
  if (currency === "USD") {
...
export default function Dashboard({ data, onRefresh }: DashboardProps) {
  ...
  const progress =
    data.monthly_summary.income > 0
      ? Math.min(
          100,
          (data.monthly_summary.savings / data.monthly_summary.income) * 100
        )
      : 0;
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-white/60 text-sm">{t("home.welcomeBack")}</p>
          <h1 className="text-2xl font-bold">
            {data.user.first_name || t("home.student")} 👋
          </h1>
        </div>
      ...
      >
        ...
        <div className="relative">
          ...
          <h2 className="text-4xl font-bold tracking-tight mb-6">
            {formatCurrency(data.balance.balance, currency, sumLabel)}
          </h2>
          ...
          <div className="flex gap-4">
            <div className="flex-1 bg-white/10 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-white/60 text-xs">{t("home.income")}</span>
              </div>
              <p className="text-lg font-semibold text-emerald-400">
                +{formatCurrency(data.monthly_summary.income, currency, sumLabel)}
              </p>
            </div>
            ...
            <div className="flex-1 bg-white/10 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-white/60 text-xs">{t("home.expense")}</span>
              </div>
              <p className="text-lg font-semibold text-rose-400">
                -{formatCurrency(data.monthly_summary.expense, currency, sumLabel)}
              </p>
            </div>
          </div>
        </div>
      ...
      {data.savings_goals.length > 0 && (
        ...
        >
          <h3 className="text-lg font-semibold mb-3">{t("wallet.activeGoals")}</h3>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/50 mt-1">
                      {formatCurrency(goal.current_amount, currency, sumLabel)} /{" "}
                      {formatCurrency(goal.target_amount, currency, sumLabel)}
                    </p>
                  </div>
                </motion.div>
              );
      ...
      >
        <h3 className="text-lg font-semibold mb-3">{t("home.recentTransactions")}</h3>

        <div className="space-y-2">
          {data.recent_transactions.length === 0 ? (
            <div className="text-center py-8 text-white/40">
              <p>{t("home.noTransactions")}</p>
              <p className="text-sm mt-1">{t("home.addFirst")}</p>
            </div>
          ) : (
            data.recent_transactions.slice(0, 5).map((transaction, index) => (
              ...
              >
                ...
                >
                  {transaction.type === "income" ? "+" : "-"}
                  {formatCurrency(transaction.amount, currency, sumLabel)}
                </span>
              </motion.div>
            ))
          )}
