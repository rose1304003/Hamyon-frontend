...
const STORAGE_KEYS = {
  ...
  SAVINGS_GOALS: "hamyon_savings_goals",
  NOTIFICATION_SETTINGS: "hamyon_notification_settings",
} as const;
...
export function getMonthlyTotals(): { income: number; expenses: number } {
  const transactions = getTransactions();
  const now = new Date();
...
export function getTotalSaved(): number {
  return getSavingsGoals().reduce((sum, goal) => sum + goal.current, 0);
}
